...
class GranolaSyncPlugin extends obsidian.Plugin {
    ...
    try {
      ...
      for (let i = 0; i < documents.length; i++) {
        ...
        try {
          ...
          if (
            (this.settings.enableDailyNoteIntegration ||
              this.settings.enablePeriodicNoteIntegration) &&
            doc.created_at
          ) {
            ...
            if (noteDate === today) {
              ...
              if (actualFile) {
                ...
                const minutes = String(createdDate.getMinutes()).padStart(
                  2,
                  "0"
                )
  ...
  async fetchTranscript(token, docId) {
    try {
      ...
      console.error(
        "Error fetching transcript for document " + docId + ":" + error
      )
      return null
    }
  }
      ...
      if (child.type === "paragraph") {
        ...
        for (const nestedItem of child.content || []) {
          if (nestedItem.type === "listItem") {
            const nestedProcessed = this.processListItem(
              nestedItem,
              indentLevel + 1
            )
            if (nestedProcessed) {
  ...
  generateFilename(doc) {
    ...
    if (doc.updated_at) {
      ...
      updatedDateTime = this.formatDate(
        doc.updated_at,
  ...
  generateDateBasedPath(doc) {
    ...
    const dateFolder = this.formatDate(
      doc.created_at,
      this.settings.dateFolderFormat
    )
  ...
  async findExistingNoteByGranolaId(granolaId) {
    ...
    if (this.settings.existingNoteSearchScope === "entireVault") {
      ...
      filesToSearch = folder.children.filter(
        file => file instanceof obsidian.TFile && file.extension === "md"
      )
    }
    ...
    for (const file of filesToSearch) {
      try {
        ...
        console.error(
          "Error reading file for Granola ID check:",
          file.path,
          error
        )
      }
  ...
  async findDuplicateNotes() {
    try {
      ...
      if (duplicates.length === 0) {
        ...
        message +=
          "Check the console for full details. You can manually delete the duplicates you don't want to keep."
      ...
      new obsidian.Notice(
        "Error finding duplicate notes. Check console for details."
      )
    }
  }
  ...
  async processDocument(doc) {
    try {
      ...
      if (existingFile) {
        if (
          this.settings.skipExistingNotes &&
          !this.settings.includeAttendeeTags &&
          !this.settings.includeGranolaUrl
        ) {
        ...
        if (
          this.settings.skipExistingNotes &&
          (this.settings.includeAttendeeTags || this.settings.includeGranolaUrl)
        ) {
    ...
    try {
      ...
      if (existingHeading) {
        ...
        for (const heading of headings) {
          if (
            heading.position.start.line > sectionLineNum &&
            heading.level <= existingHeading.level
          ) {
    ...
    try {
      ...
      const possibleDailyNotes = files.filter(file => {
        ...
        if (
          ...
          (file.name.includes(todayFormatted) ||
      ...
      possibleDailyNotes.sort((a, b) => {
        ...
        if (a.name === todayFormatted + ".md" || a.name === todayFormatted)
          return -1
        if (b.name === todayFormatted + ".md" || b.name === todayFormatted)
          return 1
      ...
      if (doc.google_calendar_event && doc.google_calendar_event.attendees) {
        for (const attendee of doc.google_calendar_event.attendees) {
          ...
          if (
            ...
            !attendees.includes(attendee.displayName)
          ) {
            ...
            attendee.email &&
            !attendees.some(name => name.includes(attendee.email.split("@")[0]))
          ) {
  ...
  generateAttendeeTags(attendees) {
    ...
    for (const attendee of attendees) {
      ...
      if (
        this.settings.excludeMyNameFromTags &&
        this.settings.myName &&
        attendee.toLowerCase().trim() ===
          this.settings.myName.toLowerCase().trim()
      ) {
        continue
      }
  ...
  findWorkspaceName(workspaceId) {
    ...
    try {
      ...
      if (Array.isArray(this.workspaces)) {
        ...
        Array.isArray(this.workspaces.workspaces)
      ) {
        ...
        this.workspaces.lists &&
        Array.isArray(this.workspaces.lists)
      ) {
  ...
  generateFolderTags(folderNames) {
    if (
      !this.settings.includeFolderTags ||
      !folderNames ||
...
class GranolaSyncSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    console.log(
      "enableGranolaFolders value:",
      this.plugin.settings.enableGranolaFolders
    )
    console.log(
      "enableGranolaFolders type:",
      typeof this.plugin.settings.enableGranolaFolders
    )
    ...
    new obsidian.Setting(containerEl)
      .setName("Date format")
      .setDesc(
        "Format for dates in filenames. Use YYYY (year), MM (month), DD (day)"
      )
      .addText(text => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        'Include the full meeting transcript in each note under a "# Transcript" section. This requires an additional API call per note and may slow down sync.'
      )
      .addToggle(toggle => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Template for filenames. Use {title}, {created_date}, {updated_date}, etc."
      )
      .addText(text => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "When enabled, notes that already exist will not be updated during sync. This preserves any manual tags, summaries, or other additions you've made."
      )
      .addToggle(toggle => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        'Format for date-based folder structure. Examples: "YYYY-MM-DD" or "YYYY/MM/DD" subfolders'
      )
      .addText(text => {
        text.setPlaceholder("YYYY/MM/DD")
    ...
    const experimentalWarning = containerEl.createEl("div", {
      cls: "setting-item",
    })
    experimentalWarning.createEl("div", { cls: "setting-item-info" })
    const warningNameEl = experimentalWarning.createEl("div", {
      cls: "setting-item-name",
    })
    warningNameEl.setText("⚠️ Please backup your vault")
    const warningDescEl = experimentalWarning.createEl("div", {
      cls: "setting-item-description",
    })
    warningDescEl.setText(
      "⚠️ The features below are experimental and may create duplicate notes if not used carefully. Please backup your vault before changing these settings."
    )

    new obsidian.Setting(containerEl)
      .setName("Search scope for existing notes")
      .setDesc(
        'Choose where to search for existing notes when checking granola-id. "Sync directory only" (default) only checks the configured sync folder. "Entire vault" allows you to move notes anywhere in your vault. "Specific folders" lets you choose which folders to search.'
      )
      .addDropdown(dropdown => {
        ...
        dropdown.onChange(async value => {
          ...
          if (oldValue !== value) {
            new obsidian.Notice(
              "Search scope changed. Consider running a manual sync to test the new settings before relying on auto-sync."
            )
          }
    ...
    if (this.plugin.settings.existingNoteSearchScope === "specificFolders") {
      new obsidian.Setting(containerEl)
        ...
        .setDesc(
          "Enter folder paths to search (one per line). Leave empty to search all folders."
        )
        .addTextArea(text => {
          ...
          text.inputEl.addEventListener("blur", () => {
            ...
            if (invalidFolders.length > 0) {
              new obsidian.Notice(
                "Warning: These folders do not exist: " +
                  invalidFolders.join(", ")
              )
            }
          })
    ...
    const infoDescEl = infoEl.createEl("div", {
      cls: "setting-item-description",
    })
    infoDescEl.setText(
      'When changing search scope, existing notes in other locations won\'t be found and may be recreated. To avoid duplicates: 1) Move your existing notes to the new search location first, or 2) Use "Entire Vault" to search everywhere, or 3) Run a manual sync after changing settings to test before auto-sync runs.'
    )
    ...
    new obsidian.Setting(containerEl)
      .setName("My name")
      .setDesc(
        "Your name as it appears in Granola meetings (used to exclude from attendee tags)"
      )
      .addText(text => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        'Customize the structure of attendee tags. Use {name} as placeholder for the attendee name. Examples: "person/{name}", "people/{name}", "meeting-attendees/{name}"'
      )
      .addText(text => {
        ...
        text.onChange(async value => {
          ...
          if (!value.includes("{name}")) {
            new obsidian.Notice(
              "Warning: Tag template should include {name} placeholder"
            )
          }
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Add Granola folder names as tags in the frontmatter of each note (requires Granola folders to be enabled)"
      )
      .addToggle(toggle => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        'Add a link back to the original Granola note in the frontmatter (e.g., granola_url: "https://notes.granola.ai/d/...")'
      )
      .addToggle(toggle => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "The heading name for the Granola meetings section in your daily note"
      )
      .addText(text => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Add todays meetings to your periodic notes (daily, weekly, or monthly - requires Periodic Notes plugin)"
      )
      .addToggle(toggle => {
    ...
    if (!this.plugin.isPeriodicNotesPluginAvailable()) {
      ...
      const warningNameEl = warningEl.createEl("div", {
        cls: "setting-item-name",
      })
      warningNameEl.setText("⚠️ Periodic Notes plugin not detected")
      const warningDescEl = warningEl.createEl("div", {
        cls: "setting-item-description",
      })
      warningDescEl.setText(
        "The Periodic Notes plugin is not installed or enabled. This integration will not work until the plugin is installed."
      )
    }
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "The heading name for the Granola meetings section in your periodic notes (works with daily, weekly, or monthly notes)"
      )
      .addText(text => {
        text.setPlaceholder("## Granola Meetings")
    ...
    const folderSetting = new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Organize notes into folders based on Granola folder structure. This will create subfolders in your sync directory for each Granola folder."
      )
      .addButton(button => {
        ...
        button.onClick(async () => {
          ...
          new obsidian.Notice(
            `Granola folders ${
              this.plugin.settings.enableGranolaFolders ? "enabled" : "disabled"
            }`
          )
        })
      })
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        'Customize the structure of folder tags. Use {name} as placeholder for the folder name. Examples: "folder/{name}", "granola/{name}", "meeting-folders/{name}"'
      )
      .addText(text => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Find and list notes with the same granola-id (helpful after changing search scope settings)"
      )
      .addButton(button => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Re-enable auto-sync after testing new search scope settings (this will restart the auto-sync timer)"
      )
      .addButton(button => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Reset Daily Notes and Periodic Notes integration to disabled (useful if toggles seem stuck)"
      )
      .addButton(button => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Reset Granola folder settings to default values (useful if toggles seem stuck)"
      )
      .addButton(button => {
